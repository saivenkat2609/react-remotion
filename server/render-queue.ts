...
export const makeRenderQueue = ({
  ...
  const processRender = async (jobId: string) => {
    ...
    try {
      const audioDuration = await getAudioDurationInSeconds(job.data.audioPath);
      const inputProps = {
        audioPath: job.data.audioPath,
        backgroundImagePath: job.data.backgroundImagePath,
        subtitleData: job.data.subtitleData,
